...
;(function(window) {
	...
	function initEvents() {
        ...
        var t101 = new MediaRevealer($('trigger-101'));
		t101.el.addEventListener(evOn, function(ev) {
			clearTimeout(triggertimeout);
			triggertimeout = setTimeout(function() {
				t101.positionMedia();
				t101.mediaEl.style.opacity = 1;
			}, triggerdelay);
		});
		t101.el.addEventListener(evOff, function(ev) {
			clearTimeout(triggertimeout);
			t101.resetMedia();
		});
        ...
        var t22 = new MediaRevealer($('trigger-22'));
		t22.el.addEventListener(evOn, function(ev) {
			clearTimeout(triggertimeout);
			triggertimeout = setTimeout(function() {
				anime.remove(t22.mediaEl);
				t22.positionMedia();
				t22.mediaEl.style.opacity = 1;	
				createShakeFx(t22.mediaEl, t22.contentEl);
			}, triggerdelay);
		});
		t22.el.addEventListener(evOff, function(ev) {
			clearTimeout(triggertimeout);
			anime.remove(t22.mediaEl);
			t22.resetMedia();
			[].slice.call(t22.contentEl.querySelectorAll('.clone')).forEach(function(clone) {
				t22.contentEl.removeChild(clone);
			});
		});
		/**************************** effect3 ****************************/
        ...
        var t33 = new MediaRevealer($('trigger-33'));
		t33.el.addEventListener(evOn, function(ev) {
			clearTimeout(triggertimeout);
			triggertimeout = setTimeout(function() {	
				anime.remove(t33.mediaEl);
				t33.positionMedia();
				anime({
					targets: t33.mediaEl,
					duration: 600,
		...
		t33.el.addEventListener(evOff, function(ev) {
			clearTimeout(triggertimeout);
			anime.remove(t33.mediaEl);
			t33.resetMedia();
		});
