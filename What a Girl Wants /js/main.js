...
;(function(window) {
	...
	function initEvents() {
        ...
        var t41 = new MediaRevealer($('trigger-41'));
		t41.el.addEventListener(evOn, function(ev) {
			clearTimeout(triggertimeout);
			triggertimeout = setTimeout(function() {
				t41.positionMedia();
				t41.mediaEl.style.opacity = 1;
				t41.mediaEl.style.WebkitTransform = t4.mediaEl.style.transform = 'rotate3d(0,0,1,-20deg)';
			}, triggerdelay);
		});
		t41.el.addEventListener(evOff, function(ev) {
			clearTimeout(triggertimeout);
			t41.resetMedia();
		});
        ...
        var t91= new MediaRevealer($('trigger-91'));
		t91.el.addEventListener(evOn, function(ev) {
			clearTimeout(triggertimeout);
			triggertimeout = setTimeout(function() {
				t91.positionMedia();
				t91.mediaEl.style.opacity = 1;
				t91.mediaEl.style.top = 0;
				t91.mediaEl.style.left = 0;
			}, triggerdelay);
		});
		t91.el.addEventListener(evOff, function(ev) {
			clearTimeout(triggertimeout);
			t91.resetMedia();
		});
        ...
        var t171 = new MediaRevealer($('trigger-171'));
		t171.el.addEventListener(evOn, function(ev) {
			clearTimeout(triggertimeout);
			triggertimeout = setTimeout(function() {
				t171.el.style.opacity = 0;
				t171.positionMedia();
				t171.mediaEl.style.opacity = 1;
				t171.mediaEl.classList.add('pop-media--show');
			}, triggerdelay);
		});
		t171.el.addEventListener(evOff, function(ev) {
			clearTimeout(triggertimeout);
			t171.el.style.opacity = 1;
			t171.resetMedia();
			t171.mediaEl.classList.remove('pop-media--show');
        });
